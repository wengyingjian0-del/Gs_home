  ...
  async function generateComic() {
    ...
    try {
      for (let index = 0; index < 4; index++) {
        ...
        const description = `四格漫画《${storyTitle}》第${index + 1}格。${panel.title}。场景：${panel.scene}。动作：${panel.action}。表情：${panel.emotion}。背景：${panel.background}。画面中不要出现任何文字、气泡或水印。严格保持主角脸、发型、服装、配饰和画风与前格一致。`;
